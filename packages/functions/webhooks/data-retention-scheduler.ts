...
import { HipaaDataRetentionManager } from '@foresight-cdss-next/webhooks';
...
const DEFAULT_RETENTION_DAYS = parseInt(process.env.DEFAULT_RETENTION_DAYS || '2555');
...
async function logRetentionAuditEvent(
  ...
  await databaseWrapper.safeInsert(() =>
    databaseWrapper.db.insert(webhookHipaaAuditLog).values({
...
async function runDataRetentionForOrganization(
  ...
  try {
    ...
    const retentionManager = new HipaaDataRetentionManager(organizationId, databaseWrapper);

    const retentionPolicies = [
      {
        name: 'webhook_deliveries_cleanup',
      ...
      {
        name: 'delivery_attempts_cleanup',
        retentionDays: DEFAULT_RETENTION_DAYS,
      },
    ];
    ...
    for (const policy of retentionPolicies) {
      ...
      console.log(`Applying retention policy: ${policy.name} (${policy.retentionDays} days)`);
      ...
      if (result.success) {
        ...
        console.log(`Retention policy ${policy.name} completed:`, {
        ...
        await logRetentionAuditEvent(
          ...
          `Applied retention policy ${policy.name}: deleted ${result.deletedRecords} records from ${result.processedTables.join(', ')}`
        );
      } else {
        console.error(`Retention policy ${policy.name} failed:`, result.errors);
        
        await sendAlert(
          'Data Retention Policy Failed',
          `Failed to apply retention policy ${policy.name} for organization ${organizationId}: ${result.errors.join(', ')}`,
          'WARNING'
        );
    ...
    await putMetric('DeletedRecords', totalDeletedRecords, 'Count', {
      OrganizationId: organizationId,
    });

    console.log(`Data retention completed for organization ${organizationId}:`, {
      totalDeletedRecords,
    });
