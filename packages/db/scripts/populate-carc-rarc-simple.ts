...
/**
 * Simple script to populate CARC/RARC adjustment reason codes from a processed SQL file.
 * This script automatically handles data insertion for the adjustment_reason_code table.
 */
...
async function getTableCounts(db: AwsDataApiPgDatabase<any>) {
  try {
    const result = await db.execute(sql`SELECT COUNT(*) as count FROM adjustment_reason_code`);

    // AWS Data API returns records as an array of Field arrays
    const count = result.records?.[0]?.[0]?.longValue ?? 0;

    return {
      adjustment_reason_codes: Number(count),
    };
  } catch (error) {
    console.error('Failed to get table counts:', error);
    return { adjustment_reason_codes: 0 };
  }
}
...
async function main() {
  try {
    ...
    console.log('🚀 Starting CARC/RARC codes population...\n');

    // Get counts before
    const beforeCounts = await getTableCounts(db);
    console.log(`📊 Current adjustment reason codes: ${beforeCounts.adjustment_reason_codes}\n`);

    // Look for SQL files (try multiple names)
    const possibleFiles = [
      'populate_carc_rarc_codes.sql',
      'carc_rarc_codes.sql',
      '../populate_carc_rarc_codes.sql',
    ];
    ...
    if (!sqlFile) {
      ...
      console.log('   python3 scripts/process-carc-rarc-codes.py');
    ...
    console.log('\n🎉 CARC/RARC codes population completed!');
    console.log('📊 Final counts:');
    console.log(`   Adjustment reason codes: ${afterCounts.adjustment_reason_codes} (added: ${afterCounts.adjustment_reason_codes - beforeCounts.adjustment_reason_codes})`);
