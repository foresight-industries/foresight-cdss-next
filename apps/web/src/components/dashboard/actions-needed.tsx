...
export function ActionsNeededCard({
  epaItems,
  claimItems,
  preEncounterIssues,
}: Readonly<ActionsNeededCardProps>) {
  ...
  const totalAwaitingReview = needsReviewClaims.reduce(
    ...
    <Card className="bg-card border border-border/60 shadow-xs">
      <CardHeader className="pb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Prioritize review queues that require manual attention
        </p>
      </CardHeader>
      <CardContent>
        <div className="grid gap-4 lg:grid-cols-3">
          <Link
            href={teamSlug ? `/team/${teamSlug}/claims` : "/claims"}
            className="group rounded-lg border border-border/60 bg-muted/20 p-4 transition hover:border-primary/40 hover:bg-muted/30"
          >
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <span>
                    {needsReviewClaims.length > 0
                      ? `${needsReviewClaims.length} items need review`
                      : "All caught up"}
                  </span>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-medium">
                          {formatCurrency(totalAwaitingReview)} total
                        </span>
                      </div>
                    </>
                  )}
            ...
            <div className="mt-4 space-y-3">
              {needsReviewClaims.length === 0 ? (
                ...
                needsReviewClaims.map((claim) => {
                  const isHighValue = isHighValueClaim(
                    claim.total_amount,
                    allClaimAmounts
                  );
                  return (
          ...
          <Link
            href={teamSlug ? `/team/${teamSlug}/queue` : "/queue"}
            className="group rounded-lg border border-border/60 bg-muted/20 p-4 transition hover:border-primary/40 hover:bg-muted/30"
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-sm font-semibold text-foreground">
                  Prior auth — Review queue
                </p>
                <p className="text-xs text-muted-foreground">
                  {needsReviewEpa.length > 0
                    ? `${needsReviewEpa.length} items need review`
                    : "All caught up"}
                </p>
              </div>
              <ArrowUpRight
          ...
          <Link
            href={
              teamSlug ? `/team/${teamSlug}/pre-encounters` : "/pre-encounters"
            }
            className="group rounded-lg border border-border/60 bg-muted/20 p-4 transition hover:border-primary/40 hover:bg-muted/30"
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-sm font-semibold text-foreground">
                  Pre-encounters — Review queue
                </p>
                <p className="text-xs text-muted-foreground">
                  {needsReviewPreEncounter.length > 0
                    ? `${needsReviewPreEncounter.length} issues need review`
                    : "All caught up"}
                </p>
              </div>
              <ArrowUpRight
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <Badge
                          variant="outline"
                          className={getIssuePriorityColor(issue.priority)}
                        >
