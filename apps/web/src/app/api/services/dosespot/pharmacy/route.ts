...
export async function GET(request: NextRequest) {
  try {
    ...
    const dosespotPharmacyId = searchParams.get('dosespotPharmacyId');
    ...
    if (!dosespotPharmacyId) {
      return Response.json(
        { 
          ...
          message: 'dosespotPharmacyId query parameter is required' 
        },
        { status: 400 }
      );
    }
    ...
    const pharmacy = await getPharmacyInfo(
      parseInt(dosespotPharmacyId),
      token.access_token
    );

    return Response.json(pharmacy);
  } catch (err) {
    console.error('Error in DoseSpot pharmacy GET endpoint:', err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { dosespotPharmacyId, organizationId } = body;
    ...
    console.error('Error in DoseSpot pharmacy POST endpoint:', err);
    const message = err instanceof Error ? err.message : 'Unknown error occurred';
