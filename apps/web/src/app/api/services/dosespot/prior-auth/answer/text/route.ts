...
type SubmitTextAnswerRequestBody = {
  providerId: number;
  caseId: number;
  questionId: number;
  answerId?: number; // Optional, may not be used for text answers
  answerText: string;
};

export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as SubmitTextAnswerRequestBody;
    const { providerId, caseId, questionId, answerText } = body;
    ...
    if (!answerText || answerText.trim() === '') {
      return Response.json(
        { message: 'answerText is required and cannot be empty' },
        { status: 400 }
      );
    }
    ...
    const priorAuthInfo = {
      ...
      url: url,
      data: {
        FreeTextAnswer: answerText.trim(),
      },
    };
