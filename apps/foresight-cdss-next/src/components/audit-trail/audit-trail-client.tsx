...
import { type DetailedAuditEntry } from '@/data/audit-trail';
...
interface AuditTrailClientProps {
  auditEntries: DetailedAuditEntry[];
}

export default function AuditTrailClient({ auditEntries }: Readonly<AuditTrailClientProps>) {
  ...
  const uniqueTableNames = useMemo(() => {
    const tables = [...new Set(auditEntries.map((entry) => entry.tableName))];
    return tables.sort();
  }, [auditEntries]);
  ...
  const filteredEntries = useMemo(() => {
    return auditEntries.filter((entry) => {
      ...
      if (
        filters.tableName !== "all" &&
        entry.tableName !== filters.tableName
      ) {
        return false;
      }
    ...
    <div className="space-y-6 p-8">
      <header className="space-y-2">
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground">
          <span>Total entries: {auditEntries.length.toLocaleString()}</span>
                  ...
                  <Badge variant="secondary" className="gap-1">
                    Entity:{" "}
                    {
                      ENTITY_TYPE_OPTIONS.find(
                        (opt) => opt.value === filters.entityType
                      )?.label
                    }
                    <Button
                      variant="ghost"
                  ...
                  <Badge variant="secondary" className="gap-1">
                    Operation:{" "}
                    {
                      OPERATION_OPTIONS.find(
                        (opt) => opt.value === filters.operation
                      )?.label
                    }
                    <Button
                      variant="ghost"
                      ...
                      className="h-auto p-0 ml-1"
                      onClick={() =>
                        setFilters((prev) => ({ ...prev, operation: "all" }))
                      }
                    >
                      <X className="w-3 h-3" />
                    </Button>
                  ...
                  <Badge variant="secondary" className="gap-1">
                    Severity:{" "}
                    {
                      SEVERITY_OPTIONS.find(
                        (opt) => opt.value === filters.severity
                      )?.label
                    }
                    <Button
                      variant="ghost"
                ...
                {filters.actor !== "all" && (
                  <Badge variant="secondary" className="gap-1">
                    ...
                    <Button
                      ...
                      className="h-auto p-0 ml-1"
                      onClick={() =>
                        setFilters((prev) => ({ ...prev, actor: "all" }))
                      }
                    >
                      <X className="w-3 h-3" />
                    </Button>
                ...
                {filters.tableName !== "all" && (
                  <Badge variant="secondary" className="gap-1">
                    ...
                    <Button
                      ...
                      className="h-auto p-0 ml-1"
                      onClick={() =>
                        setFilters((prev) => ({ ...prev, tableName: "all" }))
                      }
                    >
                      <X className="w-3 h-3" />
                    </Button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={filters.actor}
                      onValueChange={(value) =>
                        setFilters((prev) => ({ ...prev, actor: value }))
                      }
                    >
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <Badge
                            className={cn(
                              "text-xs",
                              getSeverityBadgeVariant(entry.severity)
                            )}
                          >
                            {entry.severity}
                          </Badge>
