...
import type { PreEncountersData } from '@/lib/pre-encounters-data';

interface PreEncountersClientProps {
  data: PreEncountersData;
}

export default function PreEncountersClient({ data }: Readonly<PreEncountersClientProps>) {
  const [issues, setIssues] = useState<PreEncounterIssue[]>(data.issues);
  const [selectedIssue, setSelectedIssue] = useState<PreEncounterIssue | null>(
    null
  );
  ...
  const filteredIssues = issues.filter((issue) => {
    ...
    const matchesIssueType =
      filters.issueType === "all" || issue.issueType === filters.issueType;
    const matchesPayer =
      filters.payer === "all" || issue.payerName === filters.payer;
  ...
  const handleSendInfoRequest = (issueId: string, patientName: string) => {
    ...
    setIssues((prev) =>
      prev.map((issue) =>
        issue.id === issueId
          ? {
              ...issue,
              infoRequestSentAt: new Date().toISOString(),
    ...
    toast.success(`Info request sent to ${patientName} via email/SMS`, {
      description:
        "The patient will receive notifications about updating their insurance information.",
      duration: 4000,
    });
  };
            ...
            <div className="flex items-center">
              <div className="flex-1">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  {analytics.totalUpcomingEncounters}
                </p>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {filteredIssues.length !== 1 ? "s" : ""} requiring attention
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                      ...
                      {issue.payerName && <span>Payer: {issue.payerName}</span>}
                  ...
                  <div className="flex flex-col items-end ml-4">
                    <div className="flex items-center space-x-2">
                      ...
                      {(issue.issueType === "missing_information" ||
                        issue.issueType === "expired_insurance") && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() =>
                            handleSendInfoRequest(issue.id, issue.patientName)
                          }
                      ...
                      issue.infoRequestSentAt && (
                        <div className="flex items-center space-x-2 mt-2">
                          ...
                          <span className="text-xs text-gray-500">
                            {formatDate(issue.infoRequestSentAt)} at{" "}
                            {formatTime(issue.infoRequestSentAt)}
                          </span>
                        </div>
                      )}
                  </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {formatDate(selectedIssue.appointmentDate)}
                  </p>
                </div>
                <div>
                  <Label className="text-sm font-medium">Provider</Label>
                  <p className="text-sm">
                    {selectedIssue.providerName || "N/A"}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <Badge
                    className={getIssuePriorityColor(selectedIssue.priority)}
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">
                    {formatDate(selectedIssue.createdAt)} at{" "}
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {formatDate(selectedIssue.updatedAt)} at{" "}
              ...
              {/* Info Request Status */}
              {(selectedIssue.issueType === "missing_information" ||
                selectedIssue.issueType === "expired_insurance") && (
                <div>
                  <Label className="text-sm font-medium">
                    Info Request Status
                  </Label>
                  {selectedIssue.infoRequestSentAt ? (
                    <div className="flex items-center space-x-2 mt-1">
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                        on {formatDate(selectedIssue.infoRequestSentAt)} at{" "}
                        {formatTime(selectedIssue.infoRequestSentAt)}
                      </span>
                    </div>
                  ) : (
