...
import type { ClaimsData } from '@/lib/claims-data';
...
export function ClaimsClient({ data }: Readonly<ClaimsClientProps>) {
  ...
  const [claims, setClaims] = useState<Claim[]>(() => sortClaims(data.claims));
  const [selectedClaimIds, setSelectedClaimIds] = useState<string[]>([]);
  const [activeClaimId, setActiveClaimId] = useState<string | null>(null);
  const [isClosing, setIsClosing] = useState(false);
  const closingTimeoutRef = React.useRef<NodeJS.Timeout | null>(null);
  const [submittingClaims, setSubmittingClaims] = useState<Set<string>>(
    new Set()
  );
  const [dollarFirst, setDollarFirst] = useState(true);
  const [focusedIndex, setFocusedIndex] = useState<number | null>(null);
  const [needsActionFieldValues, setNeedsActionFieldValues] = useState<
...
console.log(isClosing);
  const payers = useMemo(
    () =>
      Array.from(
        new Map(claims.map((claim) => [claim.payer.id, claim.payer]))
      ).map(([id, payer]) => ({ id, name: payer.name })),
    [claims]
  );
  ...
  const handleSaveAndContinue = (claimId: string) => {
    ...
    for (const field of fieldsNeedingAction) {
      ...
      const value =
        needsActionFieldValues[confidenceKey] ||
        needsActionFieldValues[missingKey];
      ...
      if (
        field.includes("icd") &&
        !/^[A-Z]\d{2,3}(\.\d{1,4})?$/i.test(trimmedValue)
      ) {
    ...
    if (missingFields.length > 0) {
      const fieldNames = missingFields
        .map((field) => getDescriptiveFieldName(field))
        .join(", ");
    ...
    if (invalidFields.length > 0) {
      const fieldNames = invalidFields
        ...
        .join(", ");
      toast.error(
        `Please enter valid format for: ${fieldNames}. ICD-10 codes must be complete (e.g., Z79.899).`
      );
      return;
    }
  ...
  const applyAllFixes = useCallback((ids: string[]) => {
    setClaims((prev) =>
      sortClaims(
        prev.map((claim) =>
          ids.includes(claim.id) ? applyAllFixesToClaim(claim) : claim
        )
      )
    );
  }, []);
  ...
  const applySuggestion = useCallback(
    (id: string, field: string) => {
      ...
      const fix = claim.suggested_fixes.find(
      ...
      toast.success(
        `Applied AI suggestion for ${getDescriptiveFieldName(field)}`
      );
    },
    ...
    async (
      ...
      try {
              ...
              warnings: [
                {
                  ...
                  message:
                    "Procedure code should be verified with latest updates",
                  severity: "warning",
                },
              ],
            };
        ...
        if (demoResult.status === "accepted_277ca") {
          toast.success(
            "Claim submitted successfully and accepted by clearinghouse."
          );
          ...
          toast.error(
            "Claim was rejected by clearinghouse. See errors highlighted below."
          );
        }
        ...
        setClaims((prev) => {
          return prev.map((claim) => {
            if (claim.id === claimId) {
              ...
              if (
                demoResult.status === "rejected_277ca" &&
                demoResult.errors?.length > 0
              ) {
                updatedClaim.scrubbing_result = demoResult.errors.map(
              ...
              if (demoResult.warnings && demoResult.warnings.length > 0) {
                const warnings = demoResult.warnings.map(
  ...
  const handleDenialViaPlaybook = useCallback(
    (claim: Claim) => {
      ...
      console.log(
        `Denial playbook processing claim ${claim.id} with rule ${rule.code} (${rule.strategy})`
      );
      ...
      switch (rule.strategy) {
          ...
          setTimeout(() => {
            updateClaim(claim.id, (currentClaim) => {
              const historyEntry = createDenialPlaybookHistoryEntry(
                ...
                rule.code,
                `Automatic resubmission attempt #${
                  currentClaim.attempt_count + 1
                }`
              );
              return {
                ...currentClaim,
            ...
            toast.success(
              `Claim ${claim.id} auto-resubmitted via denial playbook (rule: ${rule.code})`
            );
          }, 1000);
        ...
        case "manual_review":
          ...
          toast.info(
            `Claim ${claim.id} flagged for manual review (rule: ${rule.code})`
          );
          return true;
        ...
        case "notify":
          ...
          toast.warning(
            `Notification: Claim ${claim.id} requires attention (rule: ${rule.code})`
          );
          return true;
  ...
  useEffect(() => {
    for (const claim of claims) {
      if (
        claim.status === "denied" &&
        !processedClaimIds.current.has(claim.id)
      ) {
        const wasProcessed = handleDenialViaPlaybook(claim);
        if (
          wasProcessed ||
          !mockValidationSettings.denialPlaybook.autoRetryEnabled
        ) {
  ...
  const filteredClaims = useMemo(() => {
    let filtered = claims.filter((claim) => {
      ...
      if (
        ...
        claim.pa_status &&
        !filters.paStatuses.includes(claim.pa_status)
      ) {
        return false;
      }
      ...
      if (
        filters.claimId &&
        !claim.id.toLowerCase().includes(filters.claimId.toLowerCase())
      ) {
        return false;
      }
      ...
      if (
        ...
        !claim.payer.name
          .toLowerCase()
          .includes(filters.payerName.toLowerCase())
      ) {
        return false;
      }
      ...
      if (
        filters.provider &&
        !claim.provider.toLowerCase().includes(filters.provider.toLowerCase())
      ) {
        return false;
      }
      ...
      if (
        ...
        !claim.visit_type
          .toLowerCase()
          .includes(filters.visitType.toLowerCase())
      ) {
        return false;
      }
      ...
      if (
        filters.claimState &&
        !claim.state.toLowerCase().includes(filters.claimState.toLowerCase())
      ) {
        return false;
      }
    ...
    if (dollarFirst) {
      ...
      filtered.sort((a, b) => {
        ...
        switch (sortConfig.field) {
          ...
          default:
            aValue = String(
              a[sortConfig.field as keyof Claim] || ""
            ).toLowerCase();
            bValue = String(
              b[sortConfig.field as keyof Claim] || ""
            ).toLowerCase();
        }
  ...
  const batchSubmitDisabled =
    ...
    selectedClaimIds.some((id) => {
      ...
      return (
        ...
        claim.status === "accepted_277ca" ||
        claim.status === "awaiting_277ca"
      );
    });
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape" && activeClaimId) {
        ...
        setFocusedIndex((prev) => {
          const next =
            prev === null ? 0 : Math.min(prev + 1, filteredClaims.length - 1);
          return next;
        });
      } else if (
        ...
        (event.key === "Enter" || event.key === "o" || event.key === "O") &&
        focusedIndex !== null &&
        !activeClaimId
      ) {
        // Open focused claim
    ...
    <TooltipProvider>
      <div className="min-h-screen bg-background p-8">
            ...
            {selectedClaimIds.length > 0 && (
              <div className="flex items-center justify-between bg-muted/20 border border-border p-3 rounded-lg">
                <span className="text-sm font-medium text-foreground">
                  {selectedClaimIds.length} claim
                  {selectedClaimIds.length === 1 ? "" : "s"} selected
                </span>
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-80" align="start">
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                              <Label htmlFor="state-filter-column">
                                Filter by State
                              </Label>
                              <Input
                                ...
                                {PA_STATUS_FILTER_OPTIONS.map((option) => (
                                  ...
                                  >
                                    <Checkbox
                                      ...
                                      checked={filters.paStatuses.includes(
                                        option.value
                                      )}
                                      onCheckedChange={(checked) => {
                                        setFilters((prev) => ({
                            ...
                            <div className="flex gap-2">
                              <Button
                                ...
                                onClick={() =>
                                  setFilters((prev) => ({
                                    ...prev,
                                    paStatuses: PA_STATUS_FILTER_OPTIONS.map(
                                      (opt) => opt.value
                                    ),
                                  }))
                                }
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={(event) => {
                        ...
                        ).closest("button, a, [role=checkbox]");
                        if (interactive) {
                          return;
                        }
                    ...
                    >
                      ...
                      <TableCell className="space-y-1">
                        ...
                        {(() => {
                          const carc =
                            ...
                            claim.rejection_response?.carc ||
                            "";
                          const rarc =
                            claim.payer_response?.rarc ||
                            claim.rejection_response?.rarc ||
                            "";
                          const hasCode = carc || rarc;

                          return (claim.status === "rejected_277ca" ||
                            claim.status === "denied") &&
                            hasCode ? (
                            <Badge
                              variant="outline"
                      ...
                      <TableCell>
                        <Badge
                          className={cn(
                            "text-xs",
                            STATUS_BADGE_VARIANTS[claim.status]
                          )}
                        >
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              onClick={async (event) => {
                                ...
                                toast.success(
                                  `Claim ID ${claim.id} copied to clipboard`
                                );
                              }}
                            >
                              Copy claim #
        ...
        <ClaimDetailSheet
          ...
          onPrev={() => {
            const currentIndex = activeClaim
              ? filteredClaims.findIndex((c) => c.id === activeClaim.id)
              : -1;
            if (currentIndex > 0) {
              const prevClaim = filteredClaims[currentIndex - 1];
              handleOpenClaim(prevClaim.id);
        ...
        <Dialog open={showAddNoteDialog} onOpenChange={setShowAddNoteDialog}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Add a note that will be recorded in the claim&apos;s audit
                timeline.
              </DialogDescription>
            </DialogHeader>
